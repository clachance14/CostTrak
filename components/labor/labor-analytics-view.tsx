...
// Custom bar shape to apply opacity for forecast data
...
const CustomBar = (props: CustomBarProps) => {
  ...
  const opacity = payload.isForecast ? 0.5 : 1
...
export function LaborAnalyticsView({ projectId, jobNumber }: LaborAnalyticsViewProps) {
  ...
  const latestWeek = weeklyData.length > 0 ? weeklyData[weeklyData.length - 1].weekEnding : null
  const latestWeekEmployees = employeeData.filter(emp => emp.weekEnding === latestWeek)
  ...
  const chartData = useMemo(() => {
    const data = weeklyData.reduce((acc, week, index) => {
      ...
      const newCumulative = previousCumulative + weekTotal
      ...
      acc.push({
        ...
        cumulativeActual: !week.isForecast ? newCumulative : null,
        cumulativeForecast: null,
    ...
    // Post-process to add forecast line starting from last actual point
    const firstForecastIndex = data.findIndex(d => d.isForecast)
    if (firstForecastIndex > 0) {
      // Set the last actual point as the start of forecast line
      data[firstForecastIndex - 1].cumulativeForecast = data[firstForecastIndex - 1].cumulativeTotal
      // Fill in the forecast values
      for (let i = firstForecastIndex; i < data.length; i++) {
        data[i].cumulativeForecast = data[i].cumulativeTotal
      }
    }
  ...
  const categorySummary = useMemo(() => {
    ...
    // Get unique employees for the most recent week
    const latestWeek = weeklyData.length > 0 ? weeklyData[weeklyData.length - 1].weekEnding : null
    const latestWeekEmployees = employeeData.filter(emp => emp.weekEnding === latestWeek)
    
    latestWeekEmployees.forEach((emp) => {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url
    a.download = `labor-details-${jobNumber}-${format(new Date(), 'yyyy-MM-dd')}.csv`
    a.click()
    window.URL.revokeObjectURL(url)
  }

  const handlePivotExport = (data: Array<Record<string, unknown>>) => {
    const filename = `labor-pivot-${jobNumber}-${format(new Date(), 'yyyy-MM-dd')}.csv`
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Labor Analytics</h2>
          <p className="text-gray-600">Comprehensive labor data analysis</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>All values include 28% burden for insurance and taxes. Lighter bars indicate forecast data.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={chartData}>
                    ...
                    />
                    <Legend />
                    <Bar yAxisId="left" dataKey="directCost" fill="#2563eb" name="Direct Labor" shape={CustomBar} />
                    <Bar yAxisId="left" dataKey="indirectCost" fill="#7c3aed" name="Indirect Labor" shape={CustomBar} />
                    <Bar yAxisId="left" dataKey="staffCost" fill="#f97316" name="Staff Labor" shape={CustomBar} />
                    <Line 
                      yAxisId="right"
                      type="monotone" 
                    ...
                    <Line 
                      ...
                      strokeWidth={3}
                      strokeDasharray="5 5"
                      dot={{ fill: 'white', stroke: '#059669', strokeWidth: 2, r: 4 }}
                      connectNulls={true}
                      name=" "
                    />
                  </ComposedChart>
              ...
              {/* Custom Legend for Actual vs Forecast */}
              <div className="mt-4 flex justify-center gap-8 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700">Forecast Data</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-0.5 bg-green-600"></div>
                  <span className="text-gray-700">Actual Cumulative</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-0.5 bg-green-600 border-b-2 border-dashed border-green-600"></div>
                  <span className="text-gray-700">Forecast Cumulative</span>
                </div>
              </div>
            </CardContent>
          </Card>
