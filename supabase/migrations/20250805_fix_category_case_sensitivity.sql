-- =====================================================
-- Fix Category Case Sensitivity in Labor Functions
-- =====================================================
-- The employees table uses 'Direct', 'Indirect', 'Staff' (capitalized)
-- but the functions were checking for lowercase values
...
BEGIN
  -- Convert lowercase categories to proper case for matching
  p_categories := ARRAY(
    SELECT CASE 
      WHEN cat = 'direct' THEN 'Direct'
      WHEN cat = 'indirect' THEN 'Indirect'
      WHEN cat = 'staff' THEN 'Staff'
      ELSE cat
    END
  ...
  SELECT 
    jsonb_object_agg(
      LOWER(ct.category), -- Convert back to lowercase for API consistency
      jsonb_build_object(
        'hours', ct.hours,
        'cost', ct.cost,
...
-- =====================================================
-- 2. Fix get_labor_category_rates function
-- =====================================================
...
BEGIN
  ...
  SELECT 
    LOWER(abc.cat_name)::VARCHAR, -- Convert to lowercase for API consistency
    CASE 
...
-- =====================================================
-- 3. Fix get_weekly_actuals_by_category function
-- =====================================================
...
BEGIN
  ...
  SELECT 
    ...
    LOWER(COALESCE(e.category, 'Direct'))::VARCHAR, -- Convert to lowercase for API consistency
...
-- =====================================================
-- 4. Fix get_headcount_category_rates function
-- =====================================================
