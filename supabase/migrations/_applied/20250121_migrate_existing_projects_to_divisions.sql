...
SELECT 
  ...
  COALESCE(p.original_contract, 0) as budget_allocated,
  p.created_by,
  p.created_at
FROM projects p
...
WITH budget_aggregates AS (
  SELECT 
    ...
    (ARRAY_AGG(pbb.created_by ORDER BY pbb.created_at))[1] as created_by,
...
SELECT DISTINCT
  'project' as snapshot_type,
  pd.project_id,
  CURRENT_DATE,
  p.original_contract,
  COALESCE(co_sum.total_approved, 0),
  p.original_contract + COALESCE(co_sum.total_approved, 0),
  ...
  COALESCE(po_sum.total_committed, 0) + COALESCE(la_sum.total_actual, 0),
  CASE 
    WHEN p.original_contract > 0 
    THEN LEAST(100, (COALESCE(la_sum.total_actual, 0) / p.original_contract) * 100)
    ELSE 0 
  END
FROM project_divisions pd
